...
class CourseExporter {
	 ...
	 * @return array
	 */
	private static function get_attachment_links_from_meta( $meta_data, $meta_key ): array {
		$attachment_links = array();
		if ( is_array( $meta_data ) && count( $meta_data ) ) {
			$attachment_ids = is_array( $meta_data[ $meta_key ] ) && ! empty( $meta_data[ $meta_key ] ) ? $meta_data[ $meta_key ][0] : array();
