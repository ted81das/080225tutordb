...
class CourseImporter {
	...
	public function prepare_course_meta( $course_metas, $keep_media_files = false ) {
		...
		Importer::reset_post_data( 'video' );
		...
		if ( isset( $course_metas['_video'] ) ) {
			...
			if ( isset( $video_data['source'] ) && 'html5' === $video_data['source'] && $keep_media_files ) {
				...
				$upload_data = Importer::url_upload_file( $video_url );
		...
		Importer::reset_post_data( COURSE::COURSE_LEVEL_META );
		// Course level meta.
		...
		Importer::reset_post_data( COURSE::COURSE_SETTINGS_META );
		...
		Importer::reset_post_data( 'course_duration' );
		...
		foreach ( $additional_course_meta_mappings as $post_key => $meta_key ) {
			Importer::reset_post_data( $post_key );
			if ( isset( $course_metas[ $meta_key ] ) ) {
				$_POST[ $post_key ] = $course_metas[ $meta_key ];
			}
		}

		Importer::reset_post_data( 'tutor_course_price_type' );
		...
		Importer::reset_post_data( COURSE::COURSE_PRODUCT_ID_META );
		// Course product id.
		...
		Importer::reset_post_data( 'course_price' );
		...
		Importer::reset_post_data( 'course_sale_price' );
		...
		Importer::reset_post_data( 'course_selling_option' );
		...
		Importer::reset_post_data( 'tutor_course_certificate_template' );
		...
		Importer::reset_post_data( COURSE::COURSE_ENABLE_QA_META );
		...
		Importer::reset_post_data( COURSE::PUBLIC_COURSE_META );
		...
		Importer::reset_post_data( 'enable_coming_soon' );
		Importer::reset_post_data( 'enable_curriculum_preview' );
		...
		Importer::reset_post_data( '_is_preview' );
		// Handle lesson post meta.
		if ( isset( $course_metas['_is_preview'] ) ) {
			$_POST['_is_preview'] = $course_metas['_is_preview'];
		}
	}
}
