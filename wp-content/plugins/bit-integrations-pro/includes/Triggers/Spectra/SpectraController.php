...
class SpectraController
{
    ...
    public function getTestData()
    {
        ...
        if (!$testData || empty($testData)) {
            wp_send_json_error(new WP_Error('spectra_test', __('Spectra data is empty', 'bit-integrations-pro')));
        }
    ...
    public function removeTestData($data)
    {
        ...
        if (!$testData) {
            wp_send_json_error(new WP_Error('spectra_test', __('Failed to remove test data', 'bit-integrations-pro')));
        }
        wp_send_json_success(__('spectra test data removed successfully', 'bit-integrations-pro'));
    }
        ...
        if ($flows = Flow::exists('Spectra', current_action())) {
            foreach ($flows as $flow) {
                ...
                if ($flowDetails->primaryKey->value === $primaryKeyValue) {
                    ...
                    Flow::execute('Spectra', current_action(), $formatedData, [$flow]);
                }
            }
        }
    ...
    private static function extractValueFromPath($data, $path)
    {
        ...
        if (\is_array($data)) {
            if (!isset($data[$currentPart])) {
                wp_send_json_error(new WP_Error('Spectra', __('Index out of bounds or invalid', 'bit-integrations-pro')));
            }
        ...
        if (\is_object($data)) {
            if (!property_exists($data, $currentPart)) {
                wp_send_json_error(new WP_Error('Spectra', __('Invalid path', 'bit-integrations-pro')));
            }
